"""
week3/agents/detective.py — The Detective: role-aware retrieval from ChromaDB.
...
def detective_node(state: AppMentorState) -> dict:
    ...
    # 1. Pick chunk type based on role + question keywords
    chunk_type = _pick_chunk_type(question, role)

    # 2. Primary retrieval (filtered by chunk_type when applicable)
    chunks = _retrieve(store, question, chunk_type, n=5)

    # 3. Supplement with unfiltered results if primary was thin
    if len(chunks) < 3:
    ...
    # 4. Score confidence from cosine distances of top results
