"""
week3/agents/tracker.py — The Tracker: ReAct loop for adaptive retrieval.
...
Responsibilities:
  ...
  - Executes the chosen tool (search_commentary, search_raw_code, search_call_graph,
    search_docs, search_forum) and merges new chunks into the state
...
from week3.tools import TOOL_DEFINITIONS, execute_tool
...
def tracker_node(state: AppMentorState) -> dict:
    ...
    while tracker_retries < MAX_TRACKER_ITERATIONS and confidence < 0.6:
        ...
        if response.stop_reason == "end_turn":
            break  # Tracker judged context sufficient
        ...
        if response.stop_reason == "tool_use":
            for block in response.content:
                ...
                # Recalculate confidence from top-5 chunks
