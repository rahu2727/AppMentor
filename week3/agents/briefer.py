"""
week3/agents/briefer.py — The Briefer: persona-aware answer delivery.

Responsibilities:
  - Loads the role-specific persona prompt from week2/prompts/
...
_PROMPTS_DIR = Path(__file__).parent.parent.parent / "week2" / "prompts"
...
_FALLBACK_PERSONA = (
    "You are appMentor, a helpful ERPNext assistant. "
    "Deliver the answer clearly and concisely for the user's role."
)


def briefer_node(state: AppMentorState) -> dict:
        ...
        messages=[
            {
                ...
                "content": (
                    ...
                    f"Answer to format:\n{raw_answer}\n\n"
                    f"Sources used: {', '.join(sources) if sources else 'appMentor knowledge base'}\n\n"
